...
async def test_search_by_keyword(client: AsyncClient, search_entities):
    ...
    response = await client.get(
        "/api/search/", params={"q": "software"}, headers={"X-Product-ID": "gov"}
    )
    ...
    for item in data["data"]:
        assert (
            "software" in item["title"].lower()
            or "software" in str(item["data"]).lower()
        )
...
async def test_search_by_agency(client: AsyncClient, search_entities):
    ...
    response = await client.get(
        "/api/search/", params={"agency": "Defense"}, headers={"X-Product-ID": "gov"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] >= 1
...
async def test_search_by_naics(client: AsyncClient, search_entities):
    ...
    response = await client.get(
        "/api/search/", params={"naics_code": "541511"}, headers={"X-Product-ID": "gov"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] >= 1
...
async def test_get_recent(client: AsyncClient, search_entities):
    ...
    response = await client.get(
        "/api/search/recent", params={"limit": 10}, headers={"X-Product-ID": "gov"}
    )
    assert response.status_code == 200
    data = response.json()
    assert "data" in data
